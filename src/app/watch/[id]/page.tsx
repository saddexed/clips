    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <main className="page-container" style={{ flex: 1, maxWidth: '1000px', padding: '2rem 1rem' }}>
        ...
        <div className="glass-panel" style={{ borderRadius: 'calc(var(--radius) * 1.5)', overflow: 'hidden', background: '#000', marginBottom: '2rem', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)', border: '1px solid rgba(255,255,255,0.1)' }}>
          <SafeVideoPlayer src={`/api/videos/stream/${video.id}`} />
        </div>

        {/* Video Metadata */}
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, marginBottom: '0.5rem', color: 'var(--foreground)', letterSpacing: '-0.02em', lineHeight: 1.2 }}>
            {video.title || video.filename}
          </h1>
          
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem', marginBottom: '1rem' }}>
